...
@interface ProductListProgessView ()
...
@implementation ProductListProgessView
...
-(UILabel *)label
{
    if (!_label) {
        ...
        CGFloat y = (self.frame.size.height - height) / 2 - scaleY_6(8);
...
-(void)updateProgress:(CGFloat)progress andTerm:(NSInteger)term
{
    if (progress > 1) {
        self.label.text = @"收益中";
        self.label.textColor = kappMainTitleGray;
        progress = 0.00;
    }else{
        
        self.label.text = [NSString stringWithFormat:@"%.2f%%",progress*100];
        self.label.textColor = kappMainOrange;
        progress = progress;
    }
